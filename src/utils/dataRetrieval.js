...
export const getProjects = async () => {
    const projectDataExpanded = await apiUtil.fetchAllProjects();
...
export const getAllData = async () => {
    ...
    await Promise.all(projects.map(async (project) => {
        let issueTypesAndFieldsForProject = await apiUtil.fetchIssueTypesAndExpandFieldsByProjectKey(project.projectKey);
        if (issueTypesAndFieldsForProject.projects) {
            issueTypesAndFieldsForProject.projects[0].issuetypes.forEach(issueType => {
